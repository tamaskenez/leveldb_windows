...
bool MMap::mmap(size_t length, bool bAllowWrite, int64_t offset) {
  v_.unmapViewOfFile();
...
bool MMap::msync(void *addr, size_t length) {
...
void MMap::releaseFileHandle(winapi::File& y) {
  v_.unmapViewOfFile();
  fm_.closeHandle();
  y.moveFrom(f_);
}

bool MMap::ftruncate(int64_t size) const {
  return ::ftruncate(f_, size);
}

bool MMap::flushFileBuffers() const {
